...
int dfs1(int node) {
    ...
    dp[node] = new int[std::min(size[node], 100) + 1];
    for (int i = 2, limit = std::min(size[node], 100); i <= limit; i++) {
        dp[node][i] = neginf;
    }
    dp[node][0] = 0;
    dp[node][1] = val[node];
...
void dfs2(int node) {
    ...
    for (int &nextnode : next[node]) {
        dfs2(nextnode);
        for (int i = std::min(size[node] + size[nextnode], 100); i >= 2; i--) {
            for (int j = std::min(size[nextnode], i - 1), limit = std::max(1, i - std::min(size[node], 100)); j >= limit; j--) {
